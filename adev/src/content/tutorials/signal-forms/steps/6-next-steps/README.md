# Tebrikler!

<div class="docs-tutorial-mascot">
  <img class="docs-tutorial-mascot__angie" src="assets/images/angie/superhero.svg" aria-hidden="true">
  <p class="docs-tutorial-mascot__bubble">Başardınız! Signal Forms öğreticisini tamamladınız ve sıfırdan bir giriş formu oluşturdunuz.</p>
</div>

## Neler öğrendiniz

Bu öğretici boyunca Angular Signal Forms'un temellerini öğrendiniz:

1. **Form Models** - Sinyaller ve `form()` fonksiyonu ile tür güvenli form verisi oluşturma
2. **Field Binding** - İki yönlü veri bağlama için `[formField]` direktifini kullanma ve `value()` ile alanı görüntüleme
3. **Validation** - Özel mesajlarla yerleşik doğrulayıcılar (`required()`, `email()` gibi) uygulama
4. **Error Display** - Alan durumuna göre doğrulama hatalarını koşullu olarak gösterme
5. **Form Submission** - `submit()` fonksiyonu ile form gönderimini yönetme

## Sonraki adımlar

Daha fazla öğrenmeye hazır mısınız? İşte önerilen sonraki adımlar:

### Dokümantasyonu keşfedin

- **[Signal Forms Overview](guide/forms/signals/overview)** - Signal Forms'a giriş ve ne zaman kullanılacağı
- **[Form Models Guide](guide/forms/signals/models)** - Form modelleri ve veri yönetimi üzerine detaylı inceleme
- **[Validation Guide](guide/forms/signals/validation)** - Kapsamlı doğrulama referansları
- **[Field State Management Guide](guide/forms/signals/field-state-management)** - İleri düzey durum kalıpları
- **[Custom Controls Guide](guide/forms/signals/custom-controls)** - Yeniden kullanılabilir form bileşenleri oluşturma

## Öğrenmeye devam edin

IMPORTANT: Signal Forms deneysel aşamadadır, bu nedenle API güncellemeleri için [resmi belgelere](guide/forms/signals/overview) göz atın.

İyi kodlamalar!
