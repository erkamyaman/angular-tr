...
Signal Forms, alan ağacını modeliniz üzerinde gezinerek oluşturur. Gezindiği nesneler ve diziler (**yapısal katman**) düz JavaScript nesneleri ve dizileri olmalıdır. **Yapraklardaki** değerler (iç içe alanı olmayan konumlar) genellikle ilkel türlerdir (dizeler, sayılar, boolean'lar) veya `null`'dur. Yerel `date`, `month`, `time` ve `week` girdileri ayrıca `Date` türünü de kabul eder ve özel kontroller anladıkları herhangi bir değer türünü kabul edebilir.

```ts {prefer, header: 'Düz yapı'}
...
IMPORTANT: TypeScript bunları kabul etse de, sınıf örnekleri, `Map` ve `Set` **yapısal katmanda desteklenmez**. Signal Forms model şeklini çalışma zamanında doğrulamaz, bu nedenle framework bu değerleri hata fırlatmadan kabul eder ve ardından şekle bağlı olarak farklı şekillerde hatalı davranış üretir:

- **Sınıf örnekleri** ilk yazma işleminde prototiplerini kaybeder, çünkü Signal Forms güncelleme sırasında üst nesneleri sığ kopyalar. Metotlar, getter'lar ve `instanceof` kontrolleri sonrasında kaybolur.
- **Diziler içindeki genişletilemeyen veya dondurulmuş nesneler**, Signal Forms öğe kimliğini yeniden sıralamalar boyunca korumak için bir izleme sembolü atadığında hata fırlatır.
- **`Map` ve `Set`**, Signal Forms alt öğeleri `Object.keys` ile sıraladığından boş alan ağaçları üretir.

Uygulamanız alan modellemesi için sınıflar kullanıyorsa, form sınırında düz nesnelere dönüştürün. Bkz. [Form modeli ve alan modeli arasında dönüşüm](guide/forms/signals/model-design#form-modeli-ve-alan-modeli-arasında-dönüşüm).
