...
TIP: `@Service`, servisleri her yerde kullanılabilir hale getirir ve tree-shaking'i etkinleştirir. Tüm uygulamaya kapsamlandırmak istemiyorsanız, `autoProvided: false` belirtin.
...
- Servisin `@Service()` olduğunu veya
...
- **Birden fazla örnek mi?** Bir tekil servis birden fazla bileşen enjektöründe görünüyorsa, muhtemelen `@Service` veya `providedIn: 'root'` kullanmak yerine bileşen `providers` dizilerinde sağlanmıştır.

Bir servis hiçbir enjektörde görünmüyorsa, `@Service` dekoratörüne sahip olduğunu veya bir `providers` dizisinde listelendiğini doğrulayın.
...
- Servisin `@Service` veya `@Injectable()` dekoratörü var mı?
- `@Injectable` kullanıyorsanız, `providedIn` doğru ayarlanmış mı?
...
Angular, bağımlılık enjeksiyonu için gereken meta verileri oluşturmak üzere `@Service()` dekoratörünü gerektirir.
...
Servisi uygulamanız genelinde kullanılabilir hale getirmek için `@Service` dekoratörünü kullanın.
...
Servisin `@Service` kullandığından emin olun veya bileşenin `providers` dizisine ekleyin.
