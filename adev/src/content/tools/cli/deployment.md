...
`ng build` derlenen yapıtları varsayılan olarak `dist/my-app/` dizinine çıktılar, ancak bu yol `@angular/build:application` builder'ındaki `outputPath` seçeneğiyle yapılandırılabilir.
...
`--deploy-url`'nin etkisi ve amacı [`<base href>`](guide/routing/router-reference#base-href) ile örtüşür. Her ikisi de başlangıç betikleri, stil sayfaları, tembel betikler ve CSS kaynakları için kullanılabilir.
