...
<docs-code header="src/my-builder.ts (builder iskeleti)" path="adev/src/content/examples/cli-builder/src/my-builder.ts" region="builder-skeleton"/>

Şimdi biraz mantık ekleyelim.
Aşağıdaki kod, kaynak ve hedef dosya yollarını kullanıcı seçeneklerinden alır ve dosyayı kaynaktan hedefe kopyalar \([yerleşik Node.js `copyFile()` fonksiyonunun Promise sürümünü](https://nodejs.org/api/fs.html#fs_fspromises_copyfile_src_dest_mode) kullanarak\).
...
<docs-code header="src/my-builder.ts (çıktıyı işleme)" path="adev/src/content/examples/cli-builder/src/my-builder.ts" region="handling-output"/>

### İlerleme ve durum raporlama
...
<docs-code header="src/my-builder.ts (ilerleme raporlama)" path="adev/src/content/examples/cli-builder/src/my-builder.ts" region="progress-reporting"/>
...
<docs-code header="src/my-builder.ts (durum raporlama)" path="adev/src/content/examples/cli-builder/src/my-builder.ts" region="report-status"/>
...
Örneğin varsayılan olarak, `ng build` komutu derleme görevini gerçekleştirmek için `@angular/build:application` builder'ını çalıştırır ve `angular.json`'da `build` hedefi için belirtilen varsayılan seçenek değerlerini geçirir.
...
### Varsayılan architect yapılandırması

Hedef yapılandırmalarını bağlama yerleştiren basit bir `angular.json` dosyası oluşturalım.

Builder'ı npm'e yayınlayabilir ([Kütüphanenizi Yayınlama](tools/libraries/creating-libraries#kütüphanenizi-yayınlama) belgesine bakın) ve aşağıdaki komutla yükleyebilirsiniz:
