# HMR etkinleştirildiğinde @defer davranışı
...
Geliştirme modunda `@defer` bloğu davranışını test etmek ve gerekli bağımlılıkların tetikleme koşulu karşılandığında yüklendiğinden emin olmak istiyorsanız, [`bu belgede`](/tools/cli/build-system-migration#sıcak-modül-değiştirme) açıklandığı şekilde HMR modunu devre dışı bırakabilirsiniz.
