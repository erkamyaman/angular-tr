...
```angular-ts {avoid, header: "prefetch hiçbir zaman çalışmaz"}
@Component({
  template: `
...
```angular-ts {prefer, header: "gereksiz prefetch'i kaldırın"}
@Component({
  template: `
    @defer (on immediate) {
      <my-cmp />
...
```angular-ts {avoid, header: "prefetch ana tetikleyiciden daha geç"}
@Component({
  template: `
...
```angular-ts {avoid, header: "eşit zamanlama herhangi bir fayda sağlamaz"}
@Component({
  template: `
...
```angular-ts {prefer, header: "prefetch daha erken tetiklenir"}
@Component({
  template: `
...
```angular-ts {avoid, header: "beklenmeyen erken getirme"}
@Component({
  template: `
...
```angular-ts {prefer, header: "ana tetikleyiciyi açıkça tanımlayın"}
@Component({
  template: `
...
```angular-ts {avoid, header: "aynı viewport tetikleyicisi"}
@Component({
  template: `
...
```angular-ts {avoid, header: "aynı etkileşim hedefi"}
@Component({
  template: `
...
```angular-ts {prefer, header: "prefetch ve ana tetikleyici için farklı hedefler kullanın"}
@Component({
  template: `
