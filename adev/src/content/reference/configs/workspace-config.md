...
| `i18n`        | Proje için uluslararasılaştırma seçenekleri. Kaynak yerel ayarını ve oluşturulacak ek yerel ayarları tanımlar. [Oluşturma yapılandırmasında yerel ayarları tanımlama](guide/i18n/merge#derleme-yapılandırmasında-yerel-ayarları-tanımlama) bölümüne bakın. | [i18n seçenekleri](#i18n-seçenekleri) bölümüne bakın                                       | `{}`             |
...
Uygulamanın kaynak yerel ayarını ve oluşturulacak ek yerel ayarları tanımlamak için `i18n` proje seçeneğini kullanın.

| Özellik        | Ayrıntılar                                                                                                                                        | Değer türü                                                | Varsayılan değer |
| :------------- | :------------------------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------------- | :--------------- |
| `sourceLocale` | Uygulama kaynak kodunda kullanılan yerel ayar. Bir yerel ayar tanımlayıcı dizesi veya bir [yapılandırma nesnesi](#sourcelocale-nesnesi) olabilir. | `string` \| [sourceLocale nesnesi](#sourcelocale-nesnesi) | `"en-US"`        |
| `locales`      | Yerel ayar tanımlayıcılarını çeviri dosyalarına veya [yerel ayar yapılandırma nesnelerine](#yerel-ayar-nesnesi) eşleyen bir harita.               | `object`                                                  | `{}`             |

### `sourceLocale` nesnesi

Kaynak yerel ayar için çıktı dizinini veya temel HREF'i özelleştirmek üzere dize yerine bir nesne geçirin:

| Özellik    | Ayrıntılar                                                                                                                                         | Değer türü | Varsayılan değer |
| :--------- | :------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- | :--------------- |
| `code`     | Kaynak yerel ayar tanımlayıcısı.                                                                                                                   | `string`   | `"en-US"`        |
| `baseHref` | Bu yerel ayar için HTML `<base href>` değerini geçersiz kılar. Çıktı dizini adı yerel ayar kodu olarak kalır. `subPath` ile birlikte kullanılamaz. | `string`   | Yerel ayar kodu  |
| `subPath`  | Bu yerel ayar için hem çıktı dizini adını hem de HTML `<base href>` değerini ayarlar. `baseHref` ile birlikte kullanılamaz.                        | `string`   | Yerel ayar kodu  |

### Yerel ayar nesnesi

Her `locales` girdisi bir yol dizesi, bir yol dizisi veya bir nesne olabilir:

| Özellik       | Ayrıntılar                                                                                                                                                  | Değer türü             | Varsayılan değer |
| :------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------- | :--------------- |
| `translation` | Bu yerel ayar için çeviri dosyasının/dosyalarının yolu veya yolları.                                                                                        | `string` \| `string[]` |                  |
| `baseHref`    | Bu yerel ayar için HTML `<base href>` değerini geçersiz kılar. Çıktı dizini adı yerel ayar tanımlayıcısı olarak kalır. `subPath` ile birlikte kullanılamaz. | `string`               | Yerel ayar kodu  |
| `subPath`     | Bu yerel ayar için hem çıktı dizini adını hem de HTML `<base href>` değerini ayarlar. `baseHref` ile birlikte kullanılamaz.                                 | `string`               | Yerel ayar kodu  |

HELPFUL: Çıktı dizinini de yeniden adlandırmanız gerektiğinde, örneğin `de/` yerine `de-DE/` dizinine çıktı vermek için, `baseHref` yerine `subPath` kullanın.
...
| `inlineStyleLanguage`      | Uygulamanın satır içi bileşen stilleri için kullanılacak stil sayfası dili. `css`, `less`, `sass` veya `scss` değerlerini kabul eder, varsayılanı `css`'tir.                                                                                                                                         |
...
| Alanlar          | Ayrıntılar                                                                                                                                                            |
| :--------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `glob`           | Temel dizin olarak `input` kullanan bir [node-glob](https://github.com/isaacs/node-glob/blob/main/README.md).                                                         |
